...
| `POST /swarms/{name}/run` | SSE 流式执行 | `{input: <JSON>, context?: {...}, rounds?: number, meta_mode?: boolean}` → `event: start → run.started → node.started/node.completed/node.failed/branch.started → run.snapshot → result → done` |
...
| `GET /swarms/{name}/thought-graph` | 获取思考图（兼容别名） | 等同于 `thinking_graph` |
