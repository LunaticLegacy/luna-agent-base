...
也就是说，文件里的 `graph.py` 只是初始图，真正的执行图允许在运行中继续演化。
...
### 3.8 agent 生命周期和 node 生命周期没有强绑定
...
### 3.9 branch / join 语义在动态改图后容易失效

如果运行时删掉了 branch target 或 join target，分支语义就会坏掉。  
执行器还会尝试按照旧路径继续执行。

### 3.10 runtime_info 只负责审计，不负责调度安全
