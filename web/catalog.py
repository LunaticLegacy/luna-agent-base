...
def build_task_catalog(
    ...
    *,
    ...
    # Use TaskStore if available; fall back to legacy node-based catalog
    try:
        ...
        items = payload["items"]
        # Filter by time / query if requested
        ...
        for item in items:
            ...
            if normalized_query:
                ...
                if normalized_query not in searchable:
                    continue
            # Map task fields to legacy catalog shape for frontend compatibility
            filtered.append(
                {
                    "id": item["task_id"],
                    "name": item["name"],
    ...
    # Legacy fallback: derive tasks from execution-graph nodes + run events
