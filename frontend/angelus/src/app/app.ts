...
function formatErrorDetail(error: unknown): ErrorDetail {
  ...
  if (error instanceof HttpErrorResponse) {
    ...
    try {
      ...
      // leave body as string if not valid JSON
    }
    return {
  ...
  const stringified = typeof error === 'object' && error !== null
    ? JSON.stringify(error)
    : String(error);
  return {
    summary: stringified.length > 120 ? stringified.slice(0, 120) + '...' : stringified,
    raw: error,
    timestamp,
  };
}

function makeUserError(summary: string): ErrorDetail {
  return {
    summary,
    message: summary,
...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, FormsModule, JsonViewerComponent, GraphViewerComponent],
...
export class App {
  ...
  readonly totalAgents = computed(() => {
    return this.swarms().reduce((sum, s) => sum + s.agent_count, 0);
  });

  readonly errorCount = computed(() => {
    return this.responseFeed().filter((f) => f.tone === 'error').length;
  });
  ...
  readonly statusChips = computed<StatusItem[]>(() => {
    const index = this.apiIndex();
    const health = this.health();
    const ready = this.ready();
    const swarms = this.swarms();
    const active = this.selectedSwarmName();
    ...
    return [
      ...
      {
        label: '服务',
        value: index?.service ?? '加载中',
        tone: index?.success ? 'good' : 'pending',
      },
      {
        label: '健康',
        value: health?.status ?? '待定',
        tone: health?.success ? 'good' : 'pending',
      },
      {
        label: '就绪',
        value: ready ? (ready.ready ? '就绪' : ready.reason ?? '阻塞') : '待定',
        tone: ready ? (ready.ready ? 'good' : 'bad') : 'pending',
      },
      {
        label: 'Swarms',
        value: `${swarms.length}`,
        tone: swarms.length > 0 ? 'good' : 'pending',
      },
      {
        label: '活跃',
        value: active ?? '无',
  ...
  readonly overviewSummary = computed(() => {
    const index = this.apiIndex();
    const ready = this.ready();
    const swarmCount = this.swarms().length;
    const parts = [
      index ? `${index.service} · ${index.swarm_count} 个 Swarm` : '正在加载服务元数据',
      ready ? (ready.ready ? '运行时已就绪' : `运行时阻塞: ${ready.reason ?? '未知原因'}`) : '正在检查就绪状态',
      `${swarmCount} 已加载`,
    ];
    return parts.join(' · ');
  });

  constructor(private readonly apiService: ApiService, destroyRef: DestroyRef) {
  ...
  setApiBaseUrl(value: string): void {
    ...
    this.apiBaseUrl.set(next);
  }

  setSwarmPayloadText(value: string): void {
    this.swarmPayloadText.set(value);
  }
  ...
  setMetaMode(value: boolean): void {
    this.metaMode.set(value);
  }

  setAgentMessage(value: string): void {
    this.agentMessage.set(value);
  }
  ...
  setAdditionalPrompt(value: string): void {
    this.additionalPrompt.set(value);
  }

  setSelectedAgentId(value: string): void {
    this.selectedAgentIdChoice.set(value.trim() || null);
  }
  ...
  async loadOverview(): Promise<void> {
    ...
    try {
      ...
      if (swarmResult.status === 'fulfilled') {
        ...
        const availableNames = swarmResult.value.swarms.map((swarm) => swarm.swarm_name);
        const currentSelection = this.selectedSwarmName();
        if (!currentSelection || !availableNames.includes(currentSelection)) {
  ...
  async runSwarmSync(): Promise<void> {
    const swarmName = this.selectedSwarmName();
    if (!swarmName) {
      this.error.set(makeUserError('运行前请选择一个 Swarm。'));
      return;
    }

    let parsedPayload: unknown;
    try {
      parsedPayload = this.parseUserJson(this.swarmPayloadText());
    } catch (error) {
      this.error.set(formatErrorDetail(error));
      return;
    }
    ...
    try {
      const response = await this.apiService.runSwarm(this.baseUrl(), swarmName, {
        input: asJsonValue(parsedPayload),
      ...
      this.error.set(formatErrorDetail(error));
      this.pushFeed(`Swarm 运行失败 · ${swarmName}`, 'POST', `${this.baseUrl()}/swarms/${swarmName}/run`, 'error', {
  ...
  async startBackgroundRun(): Promise<void> {
    const swarmName = this.selectedSwarmName();
    if (!swarmName) {
      this.error.set(makeUserError('启动后台运行前请选择一个 Swarm。'));
      return;
    }
    ...
    try {
      ...
      this.error.set(formatErrorDetail(error));
      this.pushFeed(`运行启动失败 · ${swarmName}`, 'POST', `${this.baseUrl()}/swarms/${swarmName}/runs`, 'error', {
        error: errorSummary(error),
      });
    } finally {
      this.loading.set(false);
  ...
  async runAgentRound(): Promise<void> {
    ...
    if (!swarmName) {
      this.error.set(makeUserError('运行智能体前请选择一个 Swarm。'));
      return;
    }
    if (!agentId) {
      this.error.set(makeUserError('运行轮次前请选择一个智能体。'));
      return;
    }
    ...
    try {
      const response = await this.apiService.runAgentRound(this.baseUrl(), swarmName, agentId, {
        message: this.agentMessage(),
        rounds: this.agentRounds(),
        additional_prompt: this.additionalPrompt().trim() || null,
      });
      this.pushFeed(`智能体轮次 · ${agentId}`, 'POST', `${this.baseUrl()}/swarms/${swarmName}/agents/${agentId}/round`, 'success', response);
    } catch (error) {
      this.error.set(formatErrorDetail(error));
      this.pushFeed(`智能体轮次失败 · ${agentId}`, 'POST', `${this.baseUrl()}/swarms/${swarmName}/agents/${agentId}/round`, 'error', {
        error: errorSummary(error),
      });
    } finally {
      this.loading.set(false);
  ...
  selectedAgentId(): string | null {
    ...
    if (chosen && swarm?.agent_files.some((file) => this.stripAgentName(file) === chosen)) {
      return chosen;
    }
  ...
  agentChoices(): string[] {
    ...
    return swarm.agent_files.map((file) => this.stripAgentName(file));
  }
  ...
  private ensureAgentSelection(swarm: SwarmDetails): void {
    ...
    this.selectedAgentIdChoice.set(this.defaultAgentId(swarm));
  }

  private defaultAgentId(swarm: SwarmDetails | null = this.selectedSwarm()): string | null {
    if (!swarm?.agent_files?.length) {
      return null;
    }

    const preferred = swarm.agent_files.find((file) => file.includes('planner'));
    if (preferred) {
      return this.stripAgentName(preferred);
    }

    return this.stripAgentName(swarm.agent_files[0]);
  }
  ...
  private watchRun(run: RunSnapshot): void {
    ...
    source.onopen = () => {
      ...
      this.streamNote.set(`实时事件流已开启: ${run.run_id}`);
    };
    ...
    source.addEventListener('run.snapshot', (event) => {
      ...
      if (parsed) {
        ...
        this.pushLiveEvent('run.snapshot', parsed);
      }
    });
    ...
    source.addEventListener('message', (event) => {
      const parsed = this.safeParseEvent(event);
      if (parsed) {
        this.pushLiveEvent('message', parsed);
      }
    });
  ...
  private safeParseEvent(event: Event): unknown | null {
    ...
    try {
      return normalizeJsonValue(JSON.parse(message.data));
    } catch {
      return message.data;
    }
  }
  ...
  feedPreview(item: FeedItem): string {
    return valuePreview(item.payload);
  }
  ...
  swarmOverview(): string {
    ...
    return `${swarm.agent_count} 智能体 · ${swarm.skill_count} 技能 · ${swarm.tool_count} 工具`;
  }
  ...
  graphSummary(): string {
    ...
    return `${graph.node_count} 节点 · ${graph.edge_count} 边`;
  }

  payloadText(): string {
    const value = this.swarmPayloadText().trim();
    return value || '{}';
  }
  ...
  selectedRunHint(): string {
    ...
    return `${run.status} · ${run.event_count} 个事件`;
  }
